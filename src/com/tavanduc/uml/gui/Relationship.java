	...
	protected void drawConnectingPath(Graphics g, double start_length,
		...
		if (fromBound.equals(toBound)) {
			...
			if (fromBound.y + 1 < y1 && y1 < fromBound.y + fromBound.height - 1) {
				// all is good, just draw a straight line
	...
	private void drawLinePlain(Graphics g, int x1, int y1, int x2, int y2) {
		if (cfg_dash_length == 0) {
			...
			Stroke dashed = new BasicStroke(1, BasicStroke.CAP_BUTT,
					BasicStroke.JOIN_MITER, 1, new float[] { cfg_dash_length },
					0);
	...
	private PointSet drawPath(Graphics g, int x1, int y1, int x2, int y2,
		...
		if (tmp_x1 != tmp_x2 || tmp_y1 != tmp_y2) {
			...
			if (ps1.x3 != tmp_x1 || ps1.y3 != tmp_y1) {
				...
				drawPath(g, ps1.x3, ps1.y3, ps2.x2, ps2.y2, 0, 0,
						20 + 1 - mode % 2);
			}
			...
			if (ps1.x3 != tmp_x1 || ps1.y3 != tmp_y1) {
				...
				drawPath(g, ps1.x3, ps1.y3, ps3.x2, ps3.y2, 0, 0,
						20 + 1 - mode % 2);
			}
		}

		return new PointSet(x1, y1, ps1.x2, ps1.y2, ps1.delta1, ps3.x3, ps3.y3,
				x2, y2, ps3.delta2);
	}
